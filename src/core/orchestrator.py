...
def gerar_analise(transcricao: str, info_cliente: dict, doc_paths: list, progress_callback=None) -> tuple[str, str]:
    """
    Etapa 1: Lê documentos e gera a Pré-Análise e o Prompt para o protótipo.
    Retorna (pre_analise, prompt_lovable)
    """
    ...
    contexto_documentos = ""
    if doc_paths:
        report_progress(f"-> [⚙️] Lendo {len(doc_paths)} documento(s) de apoio...")
        for path in doc_paths:
            try:
                ...
                report_progress(f"-> [⚠️] Não foi possível ler o arquivo {path}: {e}")
